...
public class Half_high {
	private int XY[]=new int[97];
	private int m_XY[]=new int[97]; 
	private int E=-1;
	private int R=-1;
	private int D=-1;
	...
	public void get_XY(int VE,String product_name) throws SQLException {
		ResultSet rs=data_manager.get_XY_h_h(this.E,this.R,this.D);
		int x=0;
		while (rs.next())
	      {
			x++;
			...
			System.out.println(this.m_XY[x]);
...
//			}
		...
		data_manager.update_XY(this.m_XY,this.E,this.R,this.D,max,product_name,w-this.quantity);
