...
public class full_hight {
	
	private int XY[]=new int[2001]; 
	private int E=-1;
	private int R=-1;
	private int H=-1;
	...
	public void get_shelf(int VE,int quantity,int T_VE,String name) throws SQLException {
		...
		this.VE=VE;
		this.remainder=T_VE;
		ResultSet rs=data_manager.get_h_h_s(VE);
		while (rs.next())
	      {
	         this.E = rs.getInt("E");
	         this.R = rs.getInt("R");
	         ...
	         //System.out.println(E+" "+R+" "+H);
	         //x++;
	      }
			
		while((this.E!=-1 && this.R!=-1 && this.H!=-1)  && (this.T_VE>0)) {
			 this.E = -1;
	         this.R = -1;
	         this.H = -1;
			
			rs=data_manager.get_h_h_s(VE);
			while (rs.next())
		      {
		         this.E = rs.getInt("E");
		         this.R = rs.getInt("R");
	...
	public void get_XY(int VE,String product_name) throws SQLException {
		ResultSet rs=data_manager.get_XY_h_h(this.E,this.R,this.H);
		int x=0;
		while (rs.next())
	      {
			x++;
...
//			}
		...
		data_manager.update_XY_F(this.m_XY,this.E,this.R,this.H,max,product_name,w-this.quantity);
		this.store+="E"+E+"R"+R+"H"+H+",";
	}
